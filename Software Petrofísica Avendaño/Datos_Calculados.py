...
for Linea_M in Archivo_M.readlines():
    ...
    Nueva_Linea_Convertida_M = float(Nueva_Linea_M)
...
Archivo_DT.close()
RHOB_txt = np.array([],dtype=float)
RHOB_Aumento = 0
for Dato_M in M:
    Dato_RHOB = ((((MUD[0]-DT[RHOB_Aumento])*0.01)/Dato_M) + MUD[1])
    RHOB_txt = np.append(RHOB_txt,Dato_RHOB)
    RHOB_Aumento = RHOB_Aumento + 1
Archivo_RHOB = open("Nuevos_Archivos_Creados/RHOB.txt","w")
i=0
for Dato_RHOB in RHOB_txt:
    if i==0:
        Archivo_RHOB.writelines(str(round(Dato_RHOB,4)))
    else:
        Archivo_RHOB.writelines("\n"+str(round(Dato_RHOB,4)))
    i=i+1
Archivo_RHOB.close()
"""CALCULO DE M,N & L"""
...
Archivo_NPHI.close()
"""YA EXISTE ARCHIVO EN EL ARCHIVO EXCEL, POR ESO ESTÁ COMENTADO
